...
public class InJoinExpectingOn<T1, T2, OT, JT extends InJoinExpressionExtender<OT, JT>> {
    ...
    private final Function<ExpressionBuilder<OT, JT>, JT> joinConstructor;

    public InJoinExpectingOn(StreamOp<T1> original, StreamOp<T2> join, BiFunction<T1, T2, OT> mapper, Function<ExpressionBuilder<OT, JT>, JT> joinConstructor) {
    ...
    public <IN, OUT> GotPartialExpression<OT, OUT, InJoinExpressionExtender<OT, JT>, JT> on(String alias, Function1<IN, OUT> mapper) {
        return on(ExpressionValue.fromExtractor(MemberAccessor.from(alias, mapper)));
    }

    public <IN, OUT> GotPartialExpression<OT, OUT, InJoinExpressionExtender<OT, JT>, JT> on(Function1<IN, OUT> mapper) {
        return on(ExpressionValue.fromExtractor(MemberAccessor.from(mapper)));
    }

    public <IN, OUT> GotPartialExpression<OT, OUT, InJoinExpressionExtender<OT, JT>, JT> on(ExpressionValue<OUT> expressionValue) {
    ...
    private JT createJoinedStreamFromConditions(ExpressionBuilder<OT, JT> baseExpression) {
